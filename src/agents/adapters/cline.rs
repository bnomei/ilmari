...
/// Planned Cline CLI adapter scaffold; filtered out of `AdapterRegistry::v1`.
pub(in crate::agents) struct ClineCliAdapter;

impl AgentAdapter for ClineCliAdapter {
    fn kind(&self) -> AgentKind {
        AgentKind::ClineCli
    }

    fn detect(&self, pane: &PaneSnapshot) -> bool {
        command_matches(&pane.pane_current_command, "cline")
    }
